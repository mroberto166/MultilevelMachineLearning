...
if "diff" in keyword:
    X, y, X_test, y_test, min_val, max_val = Utils.get_data_diff(keyword, samples, variable_name, level_c, level_f, n_input,
                                                                 model_path_folder=folder_path, normalize=norm, scaler=scaler, point=point, rs=rs)
else:
...
y_pred_train = net_model.predict(X)
...
if not os.path.exists(folder_path):
    ...
    with open(folder_path + '/Score_train.txt', 'w') as file:
        file.write("MPE,SPE\n")
